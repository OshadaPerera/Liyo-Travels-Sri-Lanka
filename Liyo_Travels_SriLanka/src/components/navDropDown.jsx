  ...
  return (
    <div className="custom-dropdown" ref={dropdownRef}>
      <div className="dropdown-toggle" onClick={handleToggleDropdown}>
        {selectedLanguage.language} <img src={selectedLanguage.flag} alt={`${selectedLanguage.language} flag`} className="flag activeflag" />
      </div>
      <div className={`dropdown-menu ${isOpen ? 'active' : ''}`}>
        <div
          className={`dropdown-item ${selectedLanguage.language === 'English' ? 'active' : ''}`}
          onClick={() => handleSelect('English', UKflag)}
        >
          <img src={UKflag} alt="UK flag" className="flag" /> English
        </div>
        <div
          className={`dropdown-item ${selectedLanguage.language === 'עִברִית' ? 'active' : ''}`}
